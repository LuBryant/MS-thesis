...
\fdusetup{
  ...
  info = {
    ...
    % 如有需要，可以用 \quad 手工对齐
    %
    keywords = {量子顺磁体, 味能斯特效应, 自旋输运，贝利曲率，拓扑自旋输运。},
    % 中文关键词
    % 使用英文逗号 “,” 分隔
    %
...
\begin{abstract}
  自旋输运研究这一新兴领域揭示了量子顺磁体的引人注目的潜力，尽管它们缺乏长程磁有序，但却具备展现新奇现象的能力，并为先进技术应用开辟了道路。本论文深入探讨了量子顺磁体框架下的能斯特型热自旋输运，重点研究了自旋-轨道耦合与系统基态之间的相互作用，尽管该基态是顺磁性的，但它能够通过上支激发传递自旋信息。

  通过利用色散晶体电场（CEF）激发，我们将上支部分描述为具有不同味道的玻色子准粒子，在线性味波理论框架下进行处理。这种方法使我们能够引入味能斯特效应（FNE）的概念，这是一种拓扑自旋输运现象，区别于在有序系统中观察到的磁声子能斯特效应。

  利用包含Dzyaloshinskii-Moriya（DM）相互作用和显著硬轴各向异性的有效自旋-1哈密顿量，我们通过线性响应理论计算味能斯特系数。我们的分析进一步通过考察具有全入全出的伊辛轴配置的二维 pyrochlore 薄膜来证实，在该薄膜中，我们阐明了FNE及其对温度、各向异性、DM相互作用和外部磁场的敏感性。

  本研究的结果强调了FNE与CEF激发的贝里曲率之间的内在联系，这一关系为热自旋电流的调控和量子顺磁体中拓扑自旋输运的探索提供了前景。本研究不仅有助于量子顺磁体理论理解的深入，而且为未来实验研究奠定了基础，旨在利用这些效应实现自旋电子学领域的实际应用。
\end{abstract}
